...
@export var stage_big_image: Texture2D
...
func _ready():
	var _connect_focus = connect("focus_entered", Callable(self, "_on_button_focus_entered"))
	var _connect_pressed = connect("pressed", Callable(self, "_on_button_pressed"))
...
func _on_button_focus_entered():
	...
	TweenNode.interpolate_property(
		...
		Tween.EASE_IN )
	TweenNode.start()
	await TweenNode.tween_completed		
...
func _on_button_pressed():
	...
	var _change_scene = get_tree().change_scene_to_file(packed_level.get_path())
