...
var miniature_shader = preload("res://Shaders/BlackAndWhite.gdshader")
...
func _ready():
	...
	for node in LevelButtons:
		node.connect("mouse_entered", Callable(self, "on_mouse_entered").bind(node))
...
func _process(_delta):
	...
	if get_viewport().gui_get_focus_owner() != null:
		...
		$StageSelectCursor.global_position = focus_owner.global_position
		...
		for button in LevelButtons:
			if button == focus_owner:
				button.material.gdshader = null
			else:
				button.material.gdshader = miniature_shader
	else:
