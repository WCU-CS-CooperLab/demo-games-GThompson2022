...
func _ready():
	...
	if self.has_node("EndLevel"):
		...
		var _connect_end_level_signal = $EndLevel.connect("body_entered", Callable(self, "_on_player_finished_level").bind(), CONNECT_ONE_SHOT)
	else:
	...
	if self.has_node("Checkpoint"):
		var _connect_checkpoint = $Checkpoint.connect("body_entered", Callable(self, "_on_checkpoint_body_entered").bind(), CONNECT_ONE_SHOT)		
