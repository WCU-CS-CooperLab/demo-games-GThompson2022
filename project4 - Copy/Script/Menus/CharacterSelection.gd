...
@export var stage_selection: PackedScene
...
@onready var show_character_animation_node = $HBoxContainer/VBoxContainer2/CharacterSprite/AnimatedSprite2D
...
var glow_shader = preload("res://Shaders/GlowShader.gdshader")
...
func _ready():
	...
	for character in $HBoxContainer/VBoxContainer.get_children():
		character.connect("pressed", Callable(self, "on_character_button_pressed").bind(character))
		character.connect("mouse_entered", Callable(self, "on_mouse_entered").bind(character))
...
func _process(_delta):
	...
	if get_viewport().gui_get_focus_owner() != null:
		...
		$CharacterSelection.global_position = focus_owner.global_position
		show_character_name_node.text = get_viewport().gui_get_focus_owner().name
		...
		for index in characters:
			if index == focus_owner:
				show_character_animation_node.play(get_viewport().gui_get_focus_owner().name)
				if get_viewport().gui_get_focus_owner().name == 'Monk':
					...
					show_character_animation_node.material.gdshader = glow_shader	
					show_character_animation_node.material.set_shader_parameter("glow_color",  Color(1, 0.74902, 0, 0.286275))
				
				else:
					show_character_animation_node.material.gdshader = null		
				# We upload the points on screen based in what character is highlighted 
				_put_points(get_viewport().gui_get_focus_owner().name)
	else:
...
func on_character_button_pressed(character):
	...
	var _change_scene = get_tree().change_scene_to_file(stage_selection.get_path())
